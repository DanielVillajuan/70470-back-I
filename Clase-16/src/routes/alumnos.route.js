...
route.post('/:aid/asignar/:mid', async(req, res) => {
    ...
    const alumno = await AlumnosService.getById(aid)
    alumno.materias.push({materiaId: mid})
    const alumnoActualizado = await AlumnosService.update(aid, alumno)
    res.json({ mensaje: 'Se asigno la materia al alumno', payload: alumnoActualizado })
})
